...
import { ServiceGasBallonsService } from "./gas_ballons_service";

@ApiTags("Services_ballon")
@ApiBearerAuth()
@Controller("gas_balloons_services")
export class ServicesGasBalloonController {
  constructor(private gasBallonsService: ServiceGasBallonsService) {
  }
  ...
  async register(
    ...
    @Body() dto: BallonServiceDto, // DTO для данных
  ) {
    if (!file) {
      throw new Error("Изображение обязательно");
    }
    const imagePath = `/uploads/${file.filename}`;
    const updatedDto: BallonServiceDto = {
      ...dto,
      icon: imagePath,
    };
  ...
  async filter(
    ...
    return {
      ...
      data: result.data.map(ballon => ({
        ...ballon,
        image: `http://localhost:7000${ballon.icon}`,
      })),
    };
  }
