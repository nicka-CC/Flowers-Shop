...
import { ForKeyService} from "./for_key_service";
...
@ApiBearerAuth()
@Controller("for_key")
export class For_keyController {
  constructor(private gasBallonsService: ForKeyService) {
  }
  ...
  async register(
    ...
    @Body() dto: ForKeyDto, // DTO для данных
  ) {
    if (!file) {
      throw new Error("Изображение обязательно");
    }
    const imagePath = `/uploads/${file.filename}`;
    const updatedDto: ForKeyDto= {
      ...dto,
      image: imagePath,
    };
